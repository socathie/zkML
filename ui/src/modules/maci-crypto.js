...
const poseidonT3 = (inputs) => {
    ...
    return (0, circomlib.poseidon)(inputs);
};
...
const formatPrivKeyForBabyJub = (privKey) => {
    const sBuff = circomlib.eddsa.pruneBuffer(createBlakeHash("blake512").update(bigInt2Buffer(privKey)).digest().slice(0, 32));
...
const packPubKey = (pubKey) => {
    return circomlib.babyJub.packPoint(pubKey);
};
...
const unpackPubKey = (packed) => {
    return circomlib.babyJub.unpackPoint(packed);
};
...
const genPubKey = (privKey) => {
    ...
    assert(privKey < SNARK_FIELD_SIZE);
    return circomlib.eddsa.prv2pub(bigInt2Buffer(privKey));
};
...
const genEcdhSharedKey = (privKey, pubKey) => {
    return circomlib.babyJub.mulPointEscalar(pubKey, formatPrivKeyForBabyJub(privKey))[0];
};
...
const encrypt = (plaintext, sharedKey) => {
    // Generate the IV
    const iv = circomlib.mimc7.multiHash(plaintext, BigInt(0));
    const ciphertext = {
        iv,
        data: plaintext.map((e, i) => {
            return e + circomlib.mimc7.hash(sharedKey, iv + BigInt(i));
        }),
    };
...
const decrypt = (ciphertext, sharedKey) => {
    const plaintext = ciphertext.data.map((e, i) => {
        return BigInt(e) - BigInt(circomlib.mimc7.hash(sharedKey, BigInt(ciphertext.iv) + BigInt(i)));
    });
    return plaintext;
};
...
const sign = (privKey, msg) => {
    return circomlib.eddsa.signPoseidon(bigInt2Buffer(privKey), msg);
};
exports.sign = sign;
/*
